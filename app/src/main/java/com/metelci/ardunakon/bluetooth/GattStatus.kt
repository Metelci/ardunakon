...
object GattStatus {
     ...
     */
    fun isTransientError(status: Int): Boolean = when (status) {
    ...
    fun isPermanentError(status: Int): Boolean = when (status) {
        GATT_DEVICE_NOT_FOUND // 133 - Device gone, no point retrying
        -> true
        else -> false
    }

    fun getErrorDescription(status: Int): String = when (status) {
