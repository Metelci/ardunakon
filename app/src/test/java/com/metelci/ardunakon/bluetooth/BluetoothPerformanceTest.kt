...
class BluetoothPerformanceTest {
    ...
    fun `packet data size is reasonable for embedded devices`() {
        ...
        assertTrue(
            "Memory usage should be under 2KB, was $maxMemoryUsage bytes",
            maxMemoryUsage < 2048
        )
    }
    ...
    fun `log entry memory footprint is bounded`() {
        ...
        assertTrue(
            "Log memory should be under 50KB, was $maxLogMemory bytes",
            maxLogMemory < 51200
        )
    }
    ...
    fun `boolean flag updates are immediate`() {
        ...
        assertTrue("Boolean updates should be under 10ms for 1000 ops, was ${timeMs}ms", timeMs < 10)
    }
}
