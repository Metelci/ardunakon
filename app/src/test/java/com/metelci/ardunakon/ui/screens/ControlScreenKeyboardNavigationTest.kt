...
@Config(sdk = [34])
class ControlScreenKeyboardNavigationTest {
    ...
    @Test
    fun controlScreen_eStop_responds_to_enter_key() {
        ...
        every { bluetoothManager.setEmergencyStop(any()) } just runs
        every { bluetoothManager.disconnectAllForEStop() } just runs
        ...
        val eStopButton = composeTestRule.onNodeWithContentDescription("Emergency stop", substring = true)
        eStopButton.performSemanticsAction(SemanticsActions.RequestFocus)
        eStopButton.assertIsFocused()
        eStopButton.performKeyInput { pressKey(Key.Enter) }

        composeTestRule.runOnIdle {
            verify { bluetoothManager.setEmergencyStop(true) }
        }
    }
}
