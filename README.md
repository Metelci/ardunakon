...
## Release Info
### What's New in 0.2.27-alpha
* Current Alpha: **0.2.27-alpha** (build 58)
* **Auto-Hide Permission Tooltip**: Permission warning now automatically dismisses after 3 seconds for less intrusive UX.
* **Permission Check on Mode Switch**: Tapping WiFi or BLE mode buttons now checks for required permissions first and shows warning if denied.
* **Lock Screen Crash Fix**: Fixed crash on devices without secure lock screen (affects Samsung Galaxy S9, A10, and other devices without PIN/pattern).

### What's New in 0.2.26-alpha
* Current Alpha: **0.2.26-alpha** (build 57)
* **Permission Warning Tooltip**: App no longer blocks on infinite loading when permissions are denied - shows a friendly amber warning at the bottom of the screen instead.
* **Permission Check on Scan**: Tapping "Scan Devices" now checks permissions first and shows a warning if denied.
* **Better UX**: Users can explore the app UI even without granting permissions, with clear feedback about limited functionality.

### What's New in 0.2.25-alpha
* Current Alpha: **0.2.25-alpha** (build 56)
* **Visual Upgrade**: Servo control buttons now feature a high-contrast theme (Black text on Green/Yellow) and an electric yellow border for better visibility.
* **UX Fix**: "Quit App" now correctly stops the Bluetooth foreground service, removing the persistent notification.
* **Developer Experience**: "Security Alert" screens are now suppressed when running the app on Emulators in Debug mode.

### What's New in 0.2.23-alpha
* Current Alpha: **0.2.23-alpha** (build 54)
...
* **Lock Screen Fallback**: SecurityManager now gracefully falls back on devices without secure lock screen (fixes Samsung Galaxy A10 crash).
* **Diagnostics**: Added comprehensive startup logging for easier troubleshooting.
### What's New in 0.2.22-alpha
* Current Alpha: **0.2.22-alpha** (build 53)
* **Test Stabilization**: Fixed AndroidTest compilation issues by implementing a custom `HiltTestActivity`.
* **WiFi Reliability**: Resolved `ClassNotFoundException` in WiFi unit tests for consistent verification.
* **Coverage**: Added 100% test coverage for Util package (`RecoveryManager`, `ErrorContext`).
### What's New in 0.2.21-alpha
* Current Alpha: **0.2.21-alpha** (build 52)
...
* **Terminal Colors**: Implemented color-coded logs (TX=Green, RX=White, Error=Red) for easier debugging.
* **Stability Fixes**: Fixed joystick crash on devices with small screens or non-standard resolutions by clamping radius calculation.

### What's New in 0.2.17-alpha-hotfix1
* Current Alpha: **0.2.17-alpha-hotfix1** (build 48)
...
### What's New in 0.2.17-alpha
* Current Alpha: **0.2.17-alpha** (build 47)
...
### What's New in 0.2.16-alpha
* Current Alpha: **0.2.16-alpha** (build 46)
...
### What's New in 0.2.15-alpha
* Current Alpha: **0.2.15-alpha** (build 45)
...
### What's New in 0.2.14-alpha
* Current Alpha: **0.2.14-alpha** (build 44)
...
### What's New in 0.2.13-alpha
* Current Alpha: **0.2.13-alpha** (build 43)
...
### What's New in 0.2.10-alpha-hotfix3
